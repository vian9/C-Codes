...
// Bas ye dhyan rkhna h infinite buy and sell kr skte ho bs ind+2 ko call krna h after sell krne k baad...so sell to krdo kbhi bhi chaho but ind+2 out of bound na jaye ye check krlena beforing call
...
int maxProfit(vector<int> &prices)
{
    ...
    for (int ind = n - 1; ind >= 0; ind--)
    {
        ...
        for (int buy = 0; buy < 2; buy++)
        {
            ...
            {
                // You can either sell or skip it....agar sell kroge toh ind+1 ke jagah ind+2 kroge as restriction h
                // Sell kr skte and call bhi krdia as n+2 size ka vector lelia h
...
{
    int n = prices.size();
    // 2 Parameters h ek ind aur ek buy ya sell.....ind+2 ko call krrhe toh issliye 0 to n+1 k liye n+2 size
...
int stockSeller(vector<int> &prices, int n, int ind, int buy, vector<vector<int>> &dp)
{
    ...
    // You can either sell or skip it....agar sell kroge toh ind+1 ke jagah ind+2 kroge as restriction h
    int sell = prices[ind];
    // Ab to sell kr skte hi h even you can't call ind+2 issliye sell mein prices add krdia
    if (ind + 2 <= n)
        sell += stockSeller(prices, n, ind + 2, 1, dp);
...
int stockSeller(vector<int> &prices, int n, int ind, int buy)
{
    ...
    // Ab to sell kr skte hi h even you can't call ind+2 issliye sell mein prices add krdia
    if (ind + 2 <= n)
        sell += stockSeller(prices, n, ind + 2, 1);
