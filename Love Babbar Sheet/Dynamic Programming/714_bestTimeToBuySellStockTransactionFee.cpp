...
// Ab bs jb bhi transaction krrhe h sell ka tb hum fee subtract kr denge profit se
...
int maxProfit(vector<int> &prices, int fee)
{
    ...
    for (int ind = n - 1; ind >= 0; ind--)
    {
        ...
        for (int buy = 0; buy < 2; buy++)
        {
            ...
            {
                // You can either sell or skip it...Jb bhi sell krenge fee pay krdenge
...
// dp[ind][buy] means ki ind tk hum max profit kitna kama skte h agar ind wla ko buy krdia h ya sell krdia h
int stockSeller(vector<int> &prices, int fee, int n, int ind, int buy, vector<vector<int>> &dp)
{
    if (ind == n)
    {
        return 0;
    ...
    {
        // You can either buy or skip it
        int buyy = -prices[ind] + stockSeller(prices, fee, n, ind + 1, 0, dp);
        int notBuy = stockSeller(prices, fee, n, ind + 1, 1, dp);
        return dp[ind][buy] = max(buyy, notBuy);
    }
    // You can either sell or skip it...Jb bhi sell krenge fee pay krdenge
    int sell = prices[ind] - fee + stockSeller(prices, fee, n, ind + 1, 1, dp);
    int notSell = stockSeller(prices, fee, n, ind + 1, 0, dp);
...
int stockSeller(vector<int> &prices, int fee, int n, int ind, int buy)
{
    ...
    {
        // You can either buy or skip it
        int buyy = -prices[ind] + stockSeller(prices, fee, n, ind + 1, 0);
        int notBuy = stockSeller(prices, fee, n, ind + 1, 1);
        return max(buyy, notBuy);
    }
    // You can either sell or skip it...Jb bhi sell krenge fee pay krdenge
    int sell = prices[ind] - fee + stockSeller(prices, fee, n, ind + 1, 1);
    int notSell = stockSeller(prices, fee, n, ind + 1, 0);
...
{
    // Start 0 se hi hoga as hum buy krenge tb sell krenge
    return stockSeller(prices, fee, prices.size(), 0, 1);
}
