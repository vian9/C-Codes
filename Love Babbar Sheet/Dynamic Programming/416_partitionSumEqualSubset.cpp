...
// Approach-> Similar to subset sum GFG.....bs isme odd h sum to false and even hai to sum/2 k liye dekho agar exist krta h to thik h otherwise false
...
bool canPartition(vector<int> &nums)
{
    ...
    // Agar sum odd hai to divide krna is not possible
    if (sum % 2 == 1)
        return false;
    // even h sum toh call krdo...agar ek bhi subset with sum/2 hai to dusra apne aap ho jayega remaining subset
    sum = sum / 2;
...
bool canPartition(vector<int> &nums)
{
    ...
    // sum ko divide krdia taaki space kaam use ho
    sum = sum / 2;
    vector<vector<bool>> dp(n, vector<bool>(sum + 1, false));
    return isSubsetPos(nums, sum, dp);
}
...
bool canPartition(vector<int> &nums)
{
    ...
    // sum ko by 2 krdia h to call bs sum pr kro
    return isSubsetPos(nums, sum, n - 1, dp);
}
...
bool canPartition(vector<int> &nums)
{
    ...
    if (sum % 2 != 0)
        return false;
    // even h sum toh call krdo...agar ek bhi subset with sum/2 hai to dusra apne aap ho jayega remaining subset
    return isSubsetPos(nums, sum / 2, n - 1);
}
