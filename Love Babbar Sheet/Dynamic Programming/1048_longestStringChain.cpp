...
// Approach-> Same as LIS chain bnana h bs yhajo condition h wo yeh rhega ki hmara string mein change bs 1 ka rhe and order same rhe
// Dp[i] of LIS ka method use kro to get the max length subsequence....yha hum select nhi krrhe h balki ek extra h to move krrhe h
// So to get the length of it hum dp[i] update kro and check krte jao ki ye subsequence ka part k layak h
...
bool check(string &a, string &b)
{
    ...
    // length equal to true otherwise diff h strings
    if (a.length() == first && b.length() == second)
        return true;
    return false;
}
...
// dp[i] means the largest string chain Possible till index i in words
int longestStrChain(vector<string> &words)
{
    ...
    for (int ind = 0; ind < n; ind++)
    {
        ...
        for (int last = 0; last <= ind - 1; last++)
        {
            ...
            if (check(words[ind], words[last]) && dp[ind] < 1 + dp[last])
            {
                dp[ind] = 1 + dp[last];
            }
        }
        // Update krdia maxi subsequence ko
        if (maxi < dp[ind])
        {
            maxi = dp[ind];
        }
...
// dp[ind][last] tells us ki max string chain possible till ind and with last string chosen as last ind pr jo tha
int longestStrChain(vector<string> &words)
{
    int n = words.size();
    // Sort krdo taaki phla sequence wla hume mile
    ...
    for (int ind = n - 1; ind >= 0; ind--)
    {
        ...
        for (int last = ind - 1; last >= -1; last--)
        {
            ...
            if (last == -1 || check(words[ind], words[last]))
            {
...
// dp[ind][last] tells us ki max string chain possible till ind and with last string chosen as last ind pr jo tha
int stringHelper(vector<string> &words, int n, int ind, int last, vector<vector<int>> &dp)
{
    if (ind == n)
        return 0;
    ...
    if (last == -1 || check(words[ind], words[last]))
    {
        pick = 1 + stringHelper(words, n, ind + 1, ind, dp);
    }
    // Subsequence mein nhi pick krunga
    int notPick = 0 + stringHelper(words, n, ind + 1, last, dp);
    // return the max length possible
    // last +1 kuuki index shift kiye h ek se to store
...
int stringHelper(vector<string> &words, int n, int ind, int last)
{
    ...
    if (last == -1 || check(words[ind], words[last]))
    {
        pick = 1 + stringHelper(words, n, ind + 1, ind);
    }
    // Subsequence mein nhi pick krunga
    int notPick = 0 + stringHelper(words, n, ind + 1, last);
    // return the max length possible
...
int longestStrChain(vector<string> &words)
{
    // Sort krdo taaki phla sequence wla hume mile
    sort(words.begin(), words.end(), comp);
    return stringHelper(words, words.size(), 0, -1);
}
