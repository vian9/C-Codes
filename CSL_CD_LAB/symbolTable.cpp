...
int main()
{
    freopen("void.txt", "r", stdin);
    ifstream indata;         // indata is like cin
    string inp;              // variable for input value
    indata.open("void.txt"); // opens the file
    if (!indata)
    ...
    {                  // keep reading until end-of-file
        indata >> inp; // sets EOF flag if no value found
        cout << "The next number is " << inp << endl;
        SymbolTable st;
        string check;
        cout << "**** SYMBOL_TABLE ****\n";

        // insert 'if'
        if (st.insert(inp, "local", "keyword", 4))
            cout << " -successfully";
        else
            cout << "\nFailed to insert.\n";
        ...
        // delete 'if'
        if (st.deleteRecord(inp))
            cout << inp << " Identifier is deleted\n";
        else
            cout << "\nFailed to delete\n";
